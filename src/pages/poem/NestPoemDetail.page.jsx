...
export default function NestPoemDetailPage() {
  const shareUrl = window.location.href;
  const { nestPoemId } = useParams();
  const { useGetPoemDetailQuery } = apiHooks;
  const { data, isLoading, isError } = useGetPoemDetailQuery(nestPoemId);
  return (
    <section className=" w-full min-h-screen bg-[#faf4f8]">
